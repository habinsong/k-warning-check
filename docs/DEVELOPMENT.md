...
| 도구 | 용도 |
|------|------|
| cargo-xwin | Windows 크로스 컴파일 (macOS/Linux에서) |
| LLVM | Windows 크로스 컴파일 시 llvm-rc 필요 |
...
# macOS 빌드 → mac-app/ (ad-hoc 서명 + 격리 속성 제거 포함)
npm run build:mac

# Windows 크로스 컴파일 → windows-app/ (macOS에서 cargo-xwin + LLVM 필요)
...
### Windows 크로스 컴파일 환경 설정 (macOS)

```bash
# Rust Windows 타겟 추가
rustup target add x86_64-pc-windows-msvc

# cargo-xwin 설치 (MSVC SDK 자동 다운로드)
cargo install cargo-xwin

# LLVM 설치 (llvm-rc 리소스 컴파일러 필요)
brew install llvm

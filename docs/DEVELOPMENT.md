...
Gemini와 Groq는 저장 시 OS 보안 저장소와 런타임용 로컬 암호화 캐시를 함께 갱신합니다.
...
- Gemini/Groq 저장 뒤 분석 반복 시 암호 창이 다시 뜨지 않는지
- 분석 1회당 선택한 provider만 `analyzeRisk` 1회 호출되는지
- `history.json` 또는 확장 기록에 `llmAnalysis.responseText`가 저장되는지

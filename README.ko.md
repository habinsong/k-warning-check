...
- OCR 우선 처리와 선택한 제공자 1개만 쓰는 단일 LLM 보조 설명
- 피싱, 사기, 과장형 바이럴, AI 슬롭, 구식 정보 재탕 패턴 점수화
- Gemini, Groq, Codex 구조를 유지하되 Windows에서는 Codex 비노출
- 로컬 규칙으로 점수·유형을 고정한 뒤 LLM은 최종 설명과 근거만 1회 보강
- 기록 카드에서 제공자별 LLM 응답, 근거, 최신성 코멘트를 바로 확인
- API 키는 저장 시 OS 보안 저장소에 기록하고, 실행 중에는 로컬 암호화 캐시만 사용
...
<details>
  ...
  provider 키는 저장 시 OS 보안 저장소와 로컬 암호화 캐시에 함께 기록됩니다. 분석이나 재실행 때는 캐시만 읽으므로 매번 암호 창이 뜨지 않고, 캐시가 없으면 설정에서 다시 저장해야 합니다.
</details>

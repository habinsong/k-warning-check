...
export const RISK_BASELINES: BaselineDefinition[] = [
  {
    ...
    check: (text, hitIds) =>
      ...
      (
        hasAny(text, ['설치', '앱', '다운로드', 'install', 'app', 'download']) ||
        hitIds.includes('credential-request')
      ),
    guidance:
  ...
  },
  {
    ...
    check: (text, hitIds) =>
      ...
      hasAny(text, ['송금', '계좌이체', '대신 결제', 'transfer', 'bank transfer', 'pay on my behalf']),
    guidance:
  ...
  },
  {
    ...
    check: (text, hitIds) =>
      ...
      (hitIds.includes('impersonation') ||
        hitIds.includes('loan-repay-first') ||
        hitIds.includes('startup-grant-broker')),
    guidance:

...
function recommendedActionIds(
  ...
  const actions = new Set<RecommendedActionId>()
  ...
  if (hasMoneyRisk) {
    actions.add('verify_payment_and_business')
  }

  if (hasLinkOrAuthRisk) {
    actions.add('verify_official_site')
  }

  if (hitIds.has('external-messenger')) {
    actions.add('verify_official_channel')
  }
  ...
  if (hitIds.has('upfront-payment')) {
    actions.add('stop_upfront_payment')
  }

  if (hasAiViralityRisk) {
    actions.add('verify_ai_docs')
    actions.add('read_ai_claims_critically')
  }

  if (hitIds.has('ai-selective-comparison') || aiHookingChecklist.tags.includes('비교 왜곡')) {
    actions.add('compare_same_benchmark')
  }

  if (hitIds.has('ai-authority-trend-pressure') || aiHookingChecklist.tags.includes('권위팔이')) {
    actions.add('verify_authority_claims')
  }

  if (aiHookingChecklist.normalizedScore >= 25) {
    actions.add('verify_model_claims_and_sources')
  }

  if (highRiskGrade && (hasMoneyRisk || hasLinkOrAuthRisk)) {
    actions.add('change_password_and_contact_official_support')
  }

  if (highRiskGrade && hasAiViralityRisk && !hasMoneyRisk && !hasLinkOrAuthRisk) {
    actions.add('verify_claims_before_sharing_or_buying')
  }

  if (actions.size === 0) {
    actions.add('double_check_core_claims')
  }
...
export function generateExplanation(
  result: Omit<
    AnalysisResult,
    'summary' | 'recommendedActions' | 'summaryTemplateId' | 'recommendedActionIds'
  >,
  hits: DetectionHit[],
) {
  ...
  if (
    ...
    aiHookingChecklist.normalizedScore === 0 &&
    aiHookingChecklist.tags.length === 0
  ) {
    return {
      summaryTemplateId: 'default' as const,
      summary: '',
  ...
  let summaryTemplateId: AnalysisSummaryTemplateId = 'default'
  let summary = `${typeLead(result.primaryType)} ${gradeDescription(result.grade, 'ko')}`

  if (aiHookingChecklist.normalizedScore >= 75) {
    summaryTemplateId = 'ai_hook_high'
    summary =
      'AI 저품질 후킹글 가능성이 높습니다. 구식 정보, 제품 밀어주기, 권위팔이, 비교 왜곡, 실행 난이도 은폐 신호가 강하게 겹쳤습니다.'
  } else if (aiHookingChecklist.normalizedScore >= 50) {
    summaryTemplateId = 'ai_hook_medium'
    summary =
    ...
    summaryTemplateId = 'virality_high_scam_low'
    summary =
    ...
    summaryTemplateId = 'ai_smell_factuality'
    summary =
      'AI 딸깍형 저품질 후킹글 가능성이 높습니다. 구식 모델 정보, 빠른 성과 약속, 근거 없는 비용 프레이밍이 함께 나타납니다.'
  } else if (dimensionScores.authorityAppeal >= 60) {
    summaryTemplateId = 'authority_appeal'
    summary =
      '방향성 비판은 타당할 수 있지만 권위 호소, 내부 사례 일반화, 검증되지 않은 수치가 섞여 있어 사실 확인이 필요합니다.'
  } else if (dimensionScores.comparisonRisk >= 60) {
    summaryTemplateId = 'comparison_risk'
    summary =
      '기술 설명이라기보다 특정 제품·기기 홍보 문구에 가깝습니다. 비교 조건과 대체재 누락 여부를 비판적으로 확인해야 합니다.'
  }

  const recommendedActionIdsResult = recommendedActionIds(result.grade, hits, aiHookingChecklist)

  return {
    summaryTemplateId,
    summary,
    recommendedActionIds: recommendedActionIdsResult,
    recommendedActions: recommendedActionIdsResult.map((id) => {
      switch (id) {
        case 'verify_payment_and_business':
          return '송금, 결제, 예치금, 수수료 요청은 상대 정보와 사업자 정보를 먼저 확인하십시오.'
        case 'verify_official_site':
          return '링크를 누르지 말고 공식 사이트 주소를 직접 입력해 로그인·인증 요청이 맞는지 확인하십시오.'
        case 'verify_official_channel':
          return '외부 메신저로 이동하라는 요청은 거래·인증 흐름을 끊고 공식 채널에서 다시 확인하십시오.'
        case 'stop_upfront_payment':
          return '선입금이나 예치금 요구가 있으면 거래를 멈추고 대체 결제 수단을 확인하십시오.'
        case 'verify_ai_docs':
          return 'AI 모델명, 지원 상태, 하드웨어 조건, 비용 비교가 현재 공식 문서와 같은 기준인지 따로 확인하십시오.'
        case 'read_ai_claims_critically':
          return '광고성 문구와 기술 정보를 구분하고, 예외 조건과 한계가 빠진 문장은 비판적으로 읽으십시오.'
        case 'compare_same_benchmark':
          return '비교표가 있으면 성능, 비용, 속도, 품질 조건이 같은 기준인지 다시 맞춰 보십시오.'
        case 'verify_authority_claims':
          return '“현업은 다 이렇게 한다” 같은 문구는 출처와 실제 사례가 검증되기 전까지 업계 일반화로 보십시오.'
        case 'verify_model_claims_and_sources':
          return '모델명·버전 최신 여부, 경쟁 대안, 실제 사용 조건, 시간·비용 수치의 근거를 분리해서 확인하십시오.'
        case 'change_password_and_contact_official_support':
          return '이미 링크를 열었거나 정보를 입력했다면 비밀번호 변경과 공식 고객센터 확인을 우선 진행하십시오.'
        case 'verify_claims_before_sharing_or_buying':
          return '공유, 구매, 도입 판단 전에 원문 주장별 출처와 반례를 최소 1개 이상 확인하십시오.'
        case 'double_check_core_claims':
          return '강한 위험 신호는 낮지만, 핵심 주장과 출처가 맞는지 한 번 더 확인하십시오.'
        case 'freshness_current_keep_other_checks':
          return '최신성 자체는 웹 검색 기준으로 바로 틀렸다고 보긴 어렵습니다. 다만 과장·비교 왜곡 여부는 따로 보십시오.'
        case 'freshness_outdated_verify_model_status':
          return '현재 공식 문서 기준 모델명, 버전, 지원 상태가 맞는지 먼저 다시 확인하십시오.'
      }
    }),
  }
}
